...
| Airbnb listings (geolocated) | Inside Airbnb | region **Euskadi** snapshot **2026-06-30**: `data/listings.csv.gz` (lat/lon + attrs) + `data/reviews.csv.gz` (one row per review w/ date). Spatial-joined point→barrio (only Donostia kept) → `airbnb_density`; reviews/month → `airbnb_reviews` series. CC BY 4.0 | **wired ✓** | code ✓ |
...
| Health facilities (GIS, REC-18) | Donostia Open Data | recursos/servicios-salud/**osasunekipamenduak.json** (GeoJSON, 29 punti WGS84: ospedali, ambulatori, centri di salute). Join spaziale punto→barrio → `health_per_1000` (lato «ciudad vivida» dell'accessibilità) | **wired ✓** | code ✓ (jul-2026) |
| Municipal street directory (callejero) | Donostia Open Data — *Nombres de calle* | dataset `ce59dab6…`, zipped SHP `hiribarrukobideenizena.zip` (label **points**, EPSG:25830) with a stable street code `KodKalea` + ES/EU names. Reprojected by `gis_io`; one representative point per street. **Retires the "no callejero in the project" blocker** that parked street-grain work for VUT (REC-12) and the catastro (REC-8). Feeds `street_vut.json` / `calles_vut.csv` | **wired ✓** (street-grain, *not* joined to barrio) | code ✓ (jul-2026) |
| Touristic housing per **street** (sub-barrio) | Donostia Open Data | `vtur_censo.csv` `helbidea` addresses matched to the callejero above (longest-prefix over EU/ES name variants; 100 % of 1489 census rows) → `street_vut.json` + `calles_vut.csv`: units/beds per street at the street's label point. Powers the "Viviendas turísticas, calle a calle" map. Snapshot, point-not-line | **wired ✓** | code ✓ (jul-2026) |
| Protected housing promotions (REC-15) | Open Data Euskadi — Etxebide | ds_localizaciones/promociones_etxebide/**promociones.csv** (una riga per promozione, tutta Euskadi; coords UTM EPSG:25830 + nº alloggi nella colonna erroneamente etichettata "Tipologia"). Reproiettato + join punto→barrio → `vpo_dwellings_per_1000` (proxy: solo promozioni Etxebide, snapshot cumulativo). 13 promozioni a Donostia (1.120 alloggi). ⚠️ Finestra parziale: ≤~⅓ del solo parco protetto in affitto (3.151 unità, memoria zona tensionata 2024); il patronato municipale (Etxegintza, 2.087) non pubblica dati georeferenziati — gli «0» per barrio non sono interpretabili come assenza di VPO | **wired ✓** | code ✓ (jul-2026) |
...
| Airbnb active-listings snapshots (REC-13) | Inside Airbnb | Euskadi snapshots `visualisations/listings.csv`, **9 accessible** (quarterly 2023-12-29 → 2025-09-29 + 2026-06-30; none published in between); the 8 older ones (back to 2021-12-30) return 403 — archived-data request only. Active listings per barrio (point-in-polygon) vs reviews-LTM on the same listing universe → MET-7 quantified: supply +1.3% vs reviews +23.6% (bias ×1.22); declared-license share 58→88.5% with a −10% active drop in 2025 and **no rebound by 2026-06** (purge footprint). CC BY 4.0 | **analysis-only** (`analysis/airbnb_snapshots.py`) | code ✓ (jul-2026) |
...
| `vpo_dwellings_per_1000` — alloggi protetti Etxebide | Σ alloggi(join spaziale) / popolazione × 1000 | Etxebide promociones + demographics | **wired ✓** (REC-15; proxy, snapshot) |
| `health_per_1000` — servizi sanitari | #servizi(join spaziale) / popolazione × 1000 | salud GeoJSON + demographics | **wired ✓** (REC-18) |
