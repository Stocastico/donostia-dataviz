...
| Migration profile + employment (REC-21) | Donostia Open Data (same `demografianacionalidadbarrio.csv` as `pct_foreign`, unaggregated) + Eustat (PxWeb) — EMPA/PRA/DIRAE/I+D/renta-por-profesión | **country-of-origin by barrio is the only barrio-grain piece** (57 countries, 2000–2025). Everything about *employment* is Gipuzkoa- or C.A.-grain only: activity/employment/unemployment by continent (**pa16**) and by nationality Spanish/foreign (**cpra_tab17/18**), occupied by CNO-11 occupation (**empa_po38**), R&D personnel by occupation type (**cid_res08c**, Gipuzkoa 31‰ of employed vs. Spain 13.6‰ INE 2024 — confirms the "much more research than the Spanish average" claim), establishments by A10 sector *for Donostia* (**cdirae_est02c**), income by profession C.A.-wide (**crpf_rp_a_03**). **No table anywhere in Spain's public statistics crosses nationality × occupation × salary at municipal or even provincial grain** — checked across Eustat's full 2,321-table PxWeb catalog; this is a structural gap (small-cell suppression), not a missed source | **analysis-only** (`analysis/perfil_extranjeros_empleo.py`) | code ✓ (jul-2026) |
