...
└── procesado/      ← datos ya procesados por el pipeline (ver README interno)
    └── tablas/     ← tablas tidy CSV (metrics/series/indicators long + barrios.csv)
```
...
- **`procesado/`** — los datos que produce el pipeline. Las **tablas tidy CSV**
  (`tablas/`) ya viven aquí. El **JSON de la web** sigue en `../web/src/data/`
  porque Vite lo carga con `import.meta.glob` desde dentro de `web/src/`; ver
  `procesado/README.md` y el BACKLOG.
